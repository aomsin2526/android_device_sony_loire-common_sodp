...
static const camera_metadata_t* camera3_fixup_construct_default_request_settings(
        android::CameraMetadata metadata, int type) {
...
static int camera3_configure_streams(const camera3_device* device,
...
static void camera3_get_metadata_vendor_tag_ops(const camera3_device* device,
...
int camera3_device_open(const hw_module_t* module, const char* name, hw_device_t** device) {
    ...
    if (name != NULL) {
        ...
        rv = gVendorModule->common.methods->open((const hw_module_t*)gVendorModule, name,
                                                 (hw_device_t**)&(camera3_device->vendor));
        if (rv) {
        ...
        ALOGV("%s: got vendor camera device 0x%08X", __FUNCTION__,
              (uintptr_t)(camera3_device->vendor));
        ...
        camera3_ops->construct_default_request_settings =
