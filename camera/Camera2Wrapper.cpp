...
static const char KEY_SONY_SMILE_DETECTION[] = "sony-smile-detect";
static const char KEY_SONY_SMILE_DETECTION_VALUES[] = "sony-smile-detect-values";
...
static char* camera2_fixup_getparams(int id __unused, const char* settings) {
    ...
    if (params.get(KEY_SONY_IMAGE_STABILISER_VALUES)) {
        ...
        if (strstr(supportedIsModes, VALUE_SONY_STILL_HDR) != NULL) {
            ...
            params.set(CameraParameters::KEY_SUPPORTED_SCENE_MODES, buffer);
        }
    }
    ...
    if (params.get(KEY_SONY_SMILE_DETECTION) && params.get(KEY_SONY_SMILE_DETECTION_VALUES)) {
        params.set("face-detection-values", params.get(KEY_SONY_SMILE_DETECTION_VALUES));
        params.set("face-detection", params.get(KEY_SONY_SMILE_DETECTION));
    }
    ...
    if (params.get(KEY_SONY_ISO_MODE)) {
        if (params.get(KEY_SONY_AE_MODE_VALUES)) {
            ...
            if (strcmp(aeMode, "auto") == 0) {
                ...
                char* isoVal =
                        (char*)malloc(sizeof(char) * (3 + strlen(params.get(KEY_SONY_ISO_MODE))));
...
static char* camera2_fixup_setparams(int id __unused, const char* settings) {
    ...
    if (params.get(KEY_SONY_SMILE_DETECTION) && params.get("face-detection")) {
        params.set(KEY_SONY_SMILE_DETECTION, params.get("face-detection"));
    }

    if (params.get(CameraParameters::KEY_RECORDING_HINT) &&
        strcmp(params.get(CameraParameters::KEY_RECORDING_HINT), CameraParameters::TRUE) == 0) {
        if (params.get(KEY_SONY_VIDEO_STABILISER_VALUES) &&
            (strstr(params.get(KEY_SONY_VIDEO_STABILISER_VALUES), VALUE_SONY_INTELLIGENT_ACTIVE) !=
             NULL)) {
...
void camera2_notify_cb(int32_t msg_type, int32_t ext1, int32_t ext2, void* /*user*/) {
    gUserNotifyCb(msg_type, ext1, ext2, gUserCameraDevice);
}

void camera2_data_cb(int32_t msg_type, const camera_memory_t* data, unsigned int index,
                     camera_frame_metadata_t* metadata, void* /*user*/) {
    gUserDataCb(msg_type, data, index, metadata, gUserCameraDevice);
}

void camera2_data_cb_timestamp(nsecs_t timestamp, int32_t msg_type, const camera_memory_t* data,
                               unsigned index, void* /*user*/) {
    gUserDataCbTimestamp(timestamp, msg_type, data, index, gUserCameraDevice);
}

camera_memory_t* camera2_get_memory(int fd, size_t buf_size, uint_t num_bufs, void* /*user*/) {
    return gUserGetMemory(fd, buf_size, num_bufs, gUserCameraDevice);
}

static void camera2_set_callbacks(struct camera_device* device, camera_notify_callback notify_cb,
    ...
    VENDOR_CALL(device, set_callbacks, camera2_notify_cb, camera2_data_cb,
                camera2_data_cb_timestamp, camera2_get_memory, user);
}
...
static void camera2_release_recording_frame(struct camera_device* device, const void* opaque) {
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
          (uintptr_t)(((wrapper_camera2_device_t*)device)->vendor));
...
static int camera2_send_command(struct camera_device* device, int32_t cmd, int32_t arg1,
                                int32_t arg2) {
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
          (uintptr_t)(((wrapper_camera2_device_t*)device)->vendor));
...
int camera2_device_open(const hw_module_t* module, const char* name, hw_device_t** device) {
    ...
    if (name != NULL) {
        ...
        rv = gVendorModule->open_legacy((const hw_module_t*)gVendorModule, name,
                                        CAMERA_DEVICE_API_VERSION_1_0,
                                        (hw_device_t**)&(camera2_device->vendor));
        if (rv) {
        ...
        ALOGV("%s: got vendor camera device 0x%08X", __FUNCTION__,
              (uintptr_t)(camera2_device->vendor));
