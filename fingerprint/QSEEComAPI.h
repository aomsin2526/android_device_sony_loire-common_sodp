...
#define QSEECOM_ALIGN(x) ((x + QSEECOM_ALIGN_SIZE) & (~QSEECOM_ALIGN_MASK))
 ...
 *  error.
 */
int QSEECom_start_app(struct QSEECom_handle** clnt_handle, const char* path, const char* fname,
                      uint32_t sb_size);
 ...
 *
 */
int QSEECom_register_listener(struct QSEECom_handle** handle, uint32_t lstnr_id, uint32_t sb_length,
                              uint32_t flags);
