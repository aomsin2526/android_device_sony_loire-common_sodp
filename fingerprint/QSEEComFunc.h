...
typedef int (*start_app_def)(struct QSEECom_handle** clnt_handle, const char* path,
                             const char* fname, uint32_t sb_size);
typedef int (*shutdown_app_def)(struct QSEECom_handle** clnt_handle);
typedef int (*load_external_elf_def)(struct QSEECom_handle** clnt_handle, const char* path,
                                     const char* fname);
typedef int (*unload_external_elf_def)(struct QSEECom_handle** handle);
typedef int (*register_listener_def)(struct QSEECom_handle** handle, uint32_t lstnr_id,
                                     uint32_t sb_length, uint32_t flags);
typedef int (*unregister_listener_def)(struct QSEECom_handle* handle);
typedef int (*send_cmd_def)(struct QSEECom_handle* handle, void* send_buf, uint32_t sbuf_len,
                            void* rcv_buf, uint32_t rbuf_len);
typedef int (*send_modified_cmd_def)(struct QSEECom_handle* handle, void* send_buf,
...
typedef int32_t (*load_trustlet_def)(struct qsee_handle_t* qsee_handle,
